...
export function reducer(state: Data, action: ReducerAction): Data {
  switch (action.type) {
    ...
    case "edit":
      ...
      return addActionThen(action.payload, edit(from, to), state);
    case "delete":
...
function addTo<T, K extends DataFields>(
  entity: T,
  listName: K,
  index: number
): DataActor {
...
function move(entity: Entity, fromIndex: number, toIndex: number): DataActor {
  if (TypeUtil.isTodo(entity)) {
    return (data) => {
      ...
      data.todos = ArrayUtil.moveElement(data.todos ?? [], fromIndex, toIndex);
      return data;
    };
  } else if (TypeUtil.isLabel(entity)) {
    return (data) => {
      data.labels = ArrayUtil.moveElement(
        data.labels ?? [],
        fromIndex,
        toIndex
      );
...
// undo edit is just opposite of edit
function undoModify(action: ModifyAction): DataActor {
  return edit(action.to, action.from);
}

// edit is just delete then re-add
function edit(from: Entity, to: Entity): DataActor {
  ...
  return (d: Data) => {
    ...
    if (action) {
      ...
      switch (action.type) {
        ...
        case "edit":
          return edit(action.from, action.to)(d);
      }
    }
    return d;
  };
